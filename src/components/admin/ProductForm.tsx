...
export function ProductForm({ initialData, isEdit = false }: ProductFormProps) {
  ...
  const [targetAudience, setTargetAudience] = useState<"adult" | "kids">(
    initialData?.targetAudience || "adult",
  );
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    try {
      const totalStock =
        variants.length > 0 ? getTotalVariantStock().toString() : data.stock;
      const payload = {
        ...data,
        stock: totalStock,
        images,
        ...
        pricingTiers: pricingTiers.map((t) => ({
          ...
          maxQty:
            t.maxQty !== null && t.maxQty !== undefined
              ? Number(t.maxQty)
              : null,
      ...
      const url = isEdit
        ? `/api/store/products/${initialData.id}`
        : "/api/store/products";
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
              ...
              <label className="text-sm font-medium text-gray-700">
                ...
                {variants.length > 0 && (
                  <span className="text-xs text-gray-400">
                    (auto-calculated from variants: {getTotalVariantStock()})
                  </span>
                )}
              </label>
              <input
      ...
      {/* Target Audience & Pricing Tiers Card */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
            <Users className="w-5 h-5" /> Target Audience & Pricing
          </h3>
          <p className="text-sm text-gray-500 mt-1">
            Set the target audience and quantity-based pricing tiers
          </p>
        </div>
          ...
          <div>
            <label className="text-sm font-medium text-gray-700 block mb-3">
              ...
              <span className="text-xs text-gray-400 ml-2">
                (Leave empty to charge base price for all quantities)
              </span>
            </label>
                      ...
                      <tr key={idx} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2">
                          <input
                            ...
                            className="w-28 p-1.5 border border-gray-300 rounded text-sm text-black focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                          />
                        </td>
                        <td className="px-4 py-2">
                          <button
            ...
            <button
              ...
              onClick={() =>
                setPricingTiers([
                  ...pricingTiers,
                  { minQty: 2, maxQty: 5, pricePerUnit: 0 },
                ])
              }
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
            <Ruler className="w-5 h-5" /> Sizes &{" "}
            <Palette className="w-5 h-5" /> Colors
          </h3>
          <p className="text-sm text-gray-500 mt-1">
            Add sizes and colors to create variant combinations
          </p>
        </div>
        <div className="p-6 space-y-8">
          ...
          {variants.length > 0 && (
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-3">
                Variant Stock ({variants.length} combinations, Total:{" "}
